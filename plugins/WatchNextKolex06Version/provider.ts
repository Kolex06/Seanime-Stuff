...
function init() {
	...
	$ui.register((ctx) => {
		...
		webview.channel.sync("autoRemoveEnabled", autoRemoveEnabled);
		webview.channel.sync("autoSyncAniListEnabled", autoSyncAniListEnabled);
		...
		function saveCurrentSettings() {
			saveSettingsToStorage({
				autoRemove: autoRemoveEnabled.get(),
				autoSyncAniList: autoSyncAniListEnabled.get(),
			});
		}
		...
		function setAutoSyncAniList(value: boolean) {
			autoSyncAniListEnabled.set(value);
			saveCurrentSettings();
		}
		...
		function sendSnapshot() {
			webview.channel.send("snapshot", {
				...
				autoRemoveEnabled: autoRemoveEnabled.get(),
				autoSyncAniListEnabled: autoSyncAniListEnabled.get(),
				isSyncingAniList: isSyncingAniList.get(),
				aniListSyncStatus: aniListSyncStatus.get(),
			});
		}

		function scheduleAniListAutoSync(reason: string) {
			if (!autoSyncAniListEnabled.get()) return;
		...
		$store.watch<$app.AL_AnimeCollection>("watch-next-kolex06.latestAnimeCollection", (newCollection) => {
			if (!autoRemoveEnabled.get() || !newCollection?.MediaListCollection?.lists) return;
		...
		webview.channel.on("toggle-auto-remove", (value: boolean) => {
			setAutoRemove(!!value);
			sendSnapshot();
		});
		webview.channel.on("toggle-auto-sync-anilist", (value: boolean) => {
			const enabled = !!value;
			setAutoSyncAniList(enabled);
			if (enabled) {
				scheduleAniListAutoSync("auto-sync being enabled");
			} else {
				if (pendingAutoSyncCancel) {
					pendingAutoSyncCancel();
					pendingAutoSyncCancel = null;
				...
				<script>
					(function() {
						...
						function renderHeader(title, subtitle) {
							...
							var toggle = create("label", "toggle");
							var input = document.createElement("input");
							input.type = "checkbox";
							input.checked = !!state.autoRemoveEnabled;
							input.onchange = function() {
								state.autoRemoveEnabled = !!input.checked;
								send("toggle-auto-remove", state.autoRemoveEnabled);
								render();
							};
							toggle.appendChild(input);
							toggle.appendChild(document.createTextNode("Auto-remove CURRENT"));
							actions.appendChild(toggle);

							var syncToggle = create("label", "toggle");
							var syncInput = document.createElement("input");
							syncInput.type = "checkbox";
							syncInput.checked = !!state.autoSyncAniListEnabled;
							syncInput.onchange = function() {
								state.autoSyncAniListEnabled = !!syncInput.checked;
								send("toggle-auto-sync-anilist", state.autoSyncAniListEnabled);
								render();
							};
							syncToggle.appendChild(syncInput);
							syncToggle.appendChild(document.createTextNode("Auto-sync AniList"));
							actions.appendChild(syncToggle);
						...
						function bindWebview() {
							...
							window.webview.on("autoRemoveEnabled", function(value) {
								state.autoRemoveEnabled = !!value;
								render();
							});
							window.webview.on("autoSyncAniListEnabled", function(value) {
								state.autoSyncAniListEnabled = !!value;
								render();
							});
